...
window.loadReserve = async function(){
  ...
  tbody.innerHTML = '<tr><td colspan="7" style="padding:12px;color:#888;">加载中…</td></tr>';
  try {
    ...
    if(!has){
      tbody.innerHTML = '<tr><td colspan="7" style="padding:16px;text-align:center;color:#888;">暂无留货记录</td></tr>';
    }
    ...
    console.error(e);
    tbody.innerHTML = '<tr><td colspan="7" style="padding:16px;color:#b91c1c;">加载失败：' + ((e && e.message) || e) + '</td></tr>';
  }
};
...
function boot(){
  ...
  console.log("admin_reserve_time.js ready: 留货时间 + 超30天提醒");
}
