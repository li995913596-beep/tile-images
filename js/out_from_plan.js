...
function renderPreview(){
  ...
  html += '<thead><tr style="background:#f1f5f9;text-align:left;">' +
    ...
    '<th style="padding:8px;border-bottom:1px solid #e2e8f0;">计划数</th>' +
    '<th style="padding:8px;border-bottom:1px solid #e2e8f0;">仓库（可选）</th>' +
    '<th style="padding:8px;border-bottom:1px solid #e2e8f0;">库存/留货/可出</th>' +
    '<th style="padding:8px;border-bottom:1px solid #e2e8f0;">出库数</th>' +
  ...
  previewRows.forEach(function(row, idx){
    ...
    var opts = (row.candidates || []).map(function(c){
      var d = c.data;
      var stock = Number(d.stock || 0);
      var res = reservedTotal(d);
      var avail = Math.max(0, stock - res);
      var selected = c.id === row.invId ? " selected" : "";
      return '<option value="' + esc(c.id) + '"' + selected + '>' +
        esc(d.warehouse || "-") + "（可出" + avail + "）</option>";
    }).join("");
    var stockInfo = "-";
    ...
    html += '<tr style="background:' + bg + ';border-bottom:1px solid #f1f5f9;">' +
      ...
      '<td style="padding:8px;">' +
        (row.candidates && row.candidates.length
          ...
          : '<span style="color:#b91c1c;">无匹配库存</span>') +
      '</td>' +
      '<td style="padding:8px;font-size:12px;">' + stockInfo + '</td>' +
      '<td style="padding:8px;"><input data-opf-qty="' + idx + '" type="number" step="0.01" min="0" value="' +
        esc(row.qty) + '" style="width:80px;padding:5px 8px;border:1px solid #d1d5db;border-radius:6px;"' +
        (row.ok ? "" : " disabled") + "></td>" +
      '<td style="padding:8px;font-size:12px;color:' + (row.ok ? "#16a34a" : "#b91c1c") + ';">' +
        esc(row.error || "可出") + "</td>" +
      '<td style="padding:8px;"><button type="button" data-opf-del="' + idx +
        '" style="padding:4px 10px;border:1px solid #fecaca;background:#fee2e2;color:#b91c1c;border-radius:6px;cursor:pointer;font-size:12px;">删</button></td>' +
      "</tr>";
  });
  ...
  box.querySelectorAll("[data-opf-wh]").forEach(function(sel){
    sel.onchange = function(){
      ...
      if(hit){
        ...
        row.error = av <= 0 ? "可出为 0" : (row.qty > av ? "超过可出" : "");
        if(row.ok) row.error = "";
      }
      renderPreview();
    };
  ...
  box.querySelectorAll("[data-opf-qty]").forEach(function(inp){
    inp.onchange = inp.onblur = function(){
      ...
      row.qty = q;
      row.ok = q > 0 && av > 0;
      row.error = q <= 0 ? "数量需大于 0" : "";
      renderPreview();
    };
  });
  box.querySelectorAll("[data-opf-del]").forEach(function(btn){
    btn.onclick = function(){
      var i = Number(btn.getAttribute("data-opf-del"));
...
window.opfParsePreview = async function(){
  ...
  for(var i = 0; i < parsed.items.length; i++){
    ...
    var row = {
      ...
      invId: candidates.length ? candidates[0].id : "",
      qty: Number(it.qty) || 0,
      ok: false,
      error: ""
    };
    if(!candidates.length){
      ...
      candidates.sort(function(a, b){
        var aa = Math.max(0, Number(a.data.stock || 0) - reservedTotal(a.data));
        var bb = Math.max(0, Number(b.data.stock || 0) - reservedTotal(b.data));
        return bb - aa;
      });
      row.invId = candidates[0].id;
      row.candidates = candidates;
      var av = Math.max(0, Number(candidates[0].data.stock || 0) - reservedTotal(candidates[0].data));
      if(av <= 0){
        row.ok = false;
        row.error = "可出为 0（可能全被留货）";
        row.qty = 0;
      } else {
        if(row.qty > av) row.qty = av;
        row.ok = row.qty > 0;
  ...
  var failN = previewRows.length - okN;
  alert("识别完成：共 " + previewRows.length + " 行，可出 " + okN + " 行" + (failN ? "，异常 " + failN + " 行（红底，可删）" : ""));
};
...
window.opfConfirmOut = async function(){
  ...
  if(!lines.length) return alert("没有可出库的行（请先识别，或删除异常行后调整数量）");
  var summary = [];
  for(var i = 0; i < lines.length; i++){
    var r = lines[i];
    ...
    if(r.qty > av){
      return alert(data.code + " 色号" + (data.color || "-") + " 仓库" + data.warehouse +
        " 可出仅 " + av + "，请改小数量");
    }
  ...
  try {
    for(var j = 0; j < lines.length; j++){
      ...
      try {
        ...
        if(qty <= 0){ fail.push(L.plan.code + ": 数量无效"); continue; }
        if(qty > av || qty > Number(data.stock || 0)){ fail.push(L.plan.code + ": 可出不足"); continue; }
    ...
    var msg = "出库完成：成功 " + ok + " 行";
    if(fail.length) msg += "\n失败：\n" + fail.join("\n");
    alert(msg);
    if(ok){
      previewRows = [];
      renderPreview();
      if($("opf_text")) $("opf_text").value = "";
    }
  } finally {
...
function buildPlanPanelHtml(){
  return (
    '<div style="padding:14px;border-radius:12px;background:#f0fdfa;border:1px solid #99f6e4;margin-bottom:12px;">' +
      '<div style="font-size:13px;color:#0f766e;margin-bottom:10px;line-height:1.5;">' +
        '把「出货计划」页生成的整段文字粘贴到下方 → 识别客户/付款/账户和明细 → 每行可选仓库 → 确认后批量出库（只扣可售库存，不动留货）。' +
      '</div>' +
      ...
      '<div style="margin-top:10px;">' +
        '<button type="button" id="opf_btn_parse" style="padding:8px 16px;border:none;border-radius:8px;background:#0f766e;color:#fff;cursor:pointer;font-weight:600;">识别预览</button>' +
      '</div>' +
    '</div>' +
    '<div style="padding:14px;border-radius:12px;background:#f8fafc;border:1px solid #e2e8f0;margin-bottom:12px;">' +
      ...
      '<div style="display:flex;flex-wrap:wrap;gap:10px;align-items:center;">' +
        ...
        '<label style="font-size:13px;">账户 <input id="opf_account" placeholder="公账/私账" style="padding:6px 10px;border:1px solid #d1d5db;border-radius:8px;width:100px;"></label>' +
      '</div>' +
    '</div>' +
    '<div style="padding:14px;border-radius:12px;background:#fff;border:1px solid #e2e8f0;margin-bottom:12px;">' +
      ...
      '<div id="opf_preview"><div style="padding:8px;color:#888;font-size:13px;">识别后显示</div></div>' +
    '</div>' +
...
function enhanceOutTab(){
  var tab = $("tab_out");
  if(!tab) return false;
  if(tab.dataset.opfEnhanced === "1" && $("outPanelSingle") && $("out_search")) return true;
  tab.innerHTML =
    ...
    '<div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:14px;">' +
      ...
      '<button type="button" id="outModePlan" style="padding:8px 16px;border:none;border-radius:8px;background:#eef3f8;color:#2c3e50;cursor:pointer;font-weight:600;">计划出库</button>' +
    '</div>' +
    '<div id="outPanelSingle">' +
      ...
      '<div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:12px;align-items:center;">' +
        ...
        '<button type="button" onclick="searchOut()" style="padding:8px 16px;border:none;border-radius:8px;background:#e67e22;color:#fff;cursor:pointer;font-weight:600;">搜索</button>' +
      '</div>' +
  ...
  var bp = $("opf_btn_parse");
  if(bp) bp.onclick = function(){ window.opfParsePreview(); };
  var bc = $("opf_btn_confirm");
  if(bc) bc.onclick = function(){ window.opfConfirmOut(); };
  ...
  if(searchInput && !searchInput.__opfEnter){
    ...
    searchInput.addEventListener("keydown", function(e){
      if(e.key === "Enter"){
        e.preventDefault();
        if(typeof window.searchOut === "function") window.searchOut();
      }
    });
  }
  return true;
...
function boot(){
  ...
  var t = setInterval(function(){
    n++;
    hookShowTab();
    if($("tab_out") && $("tab_out").style.display !== "none") enhanceOutTab();
    if(n > 60) clearInterval(t);
  }, 250);
  console.log("out_from_plan.js ready v20260815a");
}
