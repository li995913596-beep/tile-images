...
function renderResPage(){
  ...
  tbody.innerHTML = "";
  if(!slice.length){
    tbody.innerHTML = "<tr><td colspan='6' style='padding:16px;text-align:center;color:#888;'>暂无留货记录</td></tr>";
  } else {
    slice.forEach(function(row){
      tbody.innerHTML += "<tr style='border-bottom:1px solid #eef2f6;'>" +
        ...
        "<td style='padding:10px 12px;'>" + esc(row.qty) + "</td>" +
        "<td style='padding:10px 12px;'>" + esc(row.customer || "") + "</td>" +
        "<td style='padding:10px 12px;'><button type='button' onclick=\"deleteReserve('" + row.id + "'," + row.index + ")\" style='padding:6px 12px;background:#fdecea;color:#e74c3c;border:none;border-radius:6px;cursor:pointer;'>取消留货</button></td></tr>";
    });
  }
...
async function loadReservePaged(){
  ...
  try {
    ...
    console.error(e);
    if(tbody) tbody.innerHTML = "<tr><td colspan='6' style='padding:12px;color:#b91c1c;'>加载失败</td></tr>";
  }
}
...
function boot(){
  ...
  console.log("admin_log_tools.js ready v20260815f");
}
