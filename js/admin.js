...
function editPanelHtml(id, i){
  return `<div style="margin-top:10px;padding:10px;background:rgba(47,125,209,0.06);border-radius:10px;border:1px solid rgba(47,125,209,0.15);">
    <div style="font-size:13px;font-weight:600;color:#2f7dd1;margin-bottom:8px;">修改信息（编号/色号/仓库改了会自动换记录）</div>
    <div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;">
      <span style="font-size:12px;color:#666;">编号</span>
      <input id="edit_code_${id}" value="${escAttr(i.code||"")}" style="width:110px;padding:6px 8px;border:1px solid #ddd;border-radius:8px;">
      <span style="font-size:12px;color:#666;">规格</span>
      <input id="edit_spec_${id}" value="${escAttr(i.spec||"")}" style="width:100px;padding:6px 8px;border:1px solid #ddd;border-radius:8px;">
      <span style="font-size:12px;color:#666;">色号</span>
      <input id="edit_color_${id}" value="${escAttr(i.color||"")}" style="width:80px;padding:6px 8px;border:1px solid #ddd;border-radius:8px;">
      <span style="font-size:12px;color:#666;">仓库</span>
      <input id="edit_wh_${id}" value="${escAttr(i.warehouse||"")}" style="width:70px;padding:6px 8px;border:1px solid #ddd;border-radius:8px;">
      <span style="font-size:12px;color:#666;">每箱片数</span>
      <input id="edit_ppb_${id}" type="number" value="${i.piecesPerBox!=null&&i.piecesPerBox!==""?escAttr(i.piecesPerBox):""}" style="width:70px;padding:6px 8px;border:1px solid #ddd;border-radius:8px;">
      <button type="button" onclick="saveEdit('${id}')" style="padding:6px 14px;border:none;border-radius:8px;background:#2f7dd1;color:#fff;cursor:pointer;">保存修改</button>
    </div>
  </div>`;
}
...
window.saveEdit = async function(id){
  try{
    ...
    const code = (($("edit_code_"+id)||{}).value || "").trim();
    const spec = (($("edit_spec_"+id)||{}).value || "").trim();
    const color = (($("edit_color_"+id)||{}).value || "").trim();
    const warehouse = (($("edit_wh_"+id)||{}).value || "").toString().trim().toLowerCase();
    const ppbRaw = (($("edit_ppb_"+id)||{}).value || "").trim();
    const piecesPerBox = ppbRaw === "" ? null : Number(ppbRaw);
    if(!code) return alert("编号不能为空");
    if(!warehouse) return alert("仓库不能为空");
    if(ppbRaw !== "" && (!piecesPerBox || piecesPerBox <= 0)) return alert("每箱片数不正确");

    const newId = code + "_" + color + "_" + warehouse;
    const same =
    ...
    if(newId === id){
      await updateDoc(ref, {
        code: code, spec: spec, color: color, warehouse: warehouse, piecesPerBox: piecesPerBox, lastUpdate: serverTimestamp()
      });
    } else {
      const exist = await getDoc(doc(db, "inventory", newId));
      if(exist.exists()){
        return alert("目标已存在：" + newId + "\n请先处理那条库存，避免重复");
      }
    ...
    await addDoc(collection(db, "logs"), {
      ...
      customer: "原:" + data.code + "_" + (data.color||"") + "_" + (data.warehouse||"")
    });
    alert("修改成功");
    if($("in_search") && $("in_search").value.trim()) searchIn();
    else if($("out_search") && $("out_search").value.trim()) searchOut();
  }catch(e){
    console.error(e);
...
function buildInPage(){
  const tab_in = $("tab_in");
  if(!tab_in) return;
  tab_in.innerHTML = `<h3>入库 / 修改信息</h3><p style="font-size:13px;color:#666;margin:0 0 8px;">搜索后可直接改编号、规格、色号、仓库。改编号/色号/仓库会自动换记录，库存和留货保留。</p><input id="in_search" placeholder="搜索编号"><button onclick="searchIn()">搜索</button><div id="in_result"></div>
...
window.searchIn = async ()=>{
  ...
  docs.forEach(d=>{ const i=d.data();
    const actions = `<div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;">数量：<input id="in_qty_${d.id}" type="number" step="0.01" style="width:100px;padding:6px 8px;border:1px solid #ddd;border-radius:8px;"><button onclick="inStock('${d.id}')" style="padding:6px 14px;border:none;border-radius:8px;background:#3498db;color:#fff;cursor:pointer;">入库</button></div>` + editPanelHtml(d.id, i);
...
window.outStock=async(id)=>{
  ...
  const customer=($("out_c_"+id).value||"").trim();
  ...
  await log("出库",data,finalQty,customer); alert("完成");
...
function buildStatsPage(){
  ...
  <button type="button" onclick="runSalesStats('custom')">查询</button>
  <span style="margin-left:8px;color:#888;">仓库</span>
  <select id="stats_warehouse" onchange="runSalesStats(adminLastPreset||'month')" style="padding:6px 10px;border-radius:8px;border:1px solid #ddd;">
...
window.runSalesStats=async function(preset){
  adminLastPreset = preset || adminLastPreset || "month";
  let start,end;
  if(preset==="custom"){
  ...
  const whFilter = (($("stats_warehouse") && $("stats_warehouse").value) || "").trim().toLowerCase();
  ...
  try{
    ...
    if(summaryEl) summaryEl.innerText=adminRankedCache.length?`共 ${totalOrders} 笔出库${whText}，总量 ${Number(totalQty.toFixed(2))}，涉及 ${adminRankedCache.length} 个编号`:`该时间段没有出库记录${whText}`;
    renderAdminStatsTable();
