...
window.loadInboundContainers = async function(){
  ...
  try {
    ...
    docs.forEach(function(d){
      ...
      if(!cn) return;
      if(!closedByCn[cn]) closedByCn[cn] = {};
      if(isClosedTransit(item)) closedByCn[cn][lineKey(item.code, item.color)] = true;
    });
    var map = {};
    docs.forEach(function(d){
      var item = d.data();
      ...
      if(!cn) return;
      if(isClosedTransit(item)) return;
      if(closedByCn[cn] && closedByCn[cn][lineKey(item.code, item.color)]) return;
      if(!map[cn]) map[cn] = 0;
      map[cn]++;
    });
...
window.loadInboundPreview = async function(){
  ...
  try {
    ...
    docs.forEach(function(d){
      var item = d.data();
      if(String(item.containerNo || "").trim() !== cn) return;
      if(isClosedTransit(item) || closed[lineKey(item.code, item.color)]){
        hidden++;
        return;
      }
    ...
    inboundCache = rows;
    if(!rows.length){
      box.innerHTML = '<div style="color:#b91c1c;">该柜没有待入库行' + (hidden ? "（已隐藏 " + hidden + " 行已入库/重复记录）" : "（可能已入库）") + '</div>';
      return;
    }
    ...
    html += '<thead><tr style="background:#f1f5f9;text-align:left;">' +
      ...
      '<th style="padding:8px;border-bottom:1px solid #e2e8f0;">箱采kg</th>' +
    ...
    html += '<div style="margin-top:10px;font-size:12px;color:#64748b;">待入库 ' + rows.length + ' 行' + (hidden ? "，已自动隐藏 " + hidden + " 行已入库/重复记录" : "") + '。规格自动规范成 600x1200。实入 = 数量 − 损坏。</div>';
    html += '<div style="margin-top:8px;"><button type="button" id="btnInboundMarkOnly" style="padding:7px 12px;border:1px solid #d1d5db;border-radius:8px;background:#fff;color:#374151;cursor:pointer;font-size:13px;">这些已入过库，只标记已入库（不增加库存）</button></div>';
    box.innerHTML = html;
    var markBtn = $("btnInboundMarkOnly");
    if(markBtn) markBtn.onclick = function(){ window.markContainerInboundOnly(); };
  } catch(e){
    console.error(e);
...
window.markContainerInboundOnly = async function(){
  ...
  if(!confirm("确认把本柜这 " + inboundCache.length + " 行只改成「已入库」？\n不会增加库存，只是从整柜入库列表里拿掉。")) return;
  var ok = 0, fail = [];
  for(var i = 0; i < inboundCache.length; i++){
    try {
      ...
      fail.push((inboundCache[i].item && inboundCache[i].item.code) || inboundCache[i].id);
    }
  }
  alert("已标记 " + ok + " 行" + (fail.length ? "\n失败：" + fail.join("、") : ""));
  inboundCache = [];
  if($("inbound_preview")) $("inbound_preview").innerHTML = "";
  window.loadInboundContainers();
  if(window.reloadTransitAdmin) window.reloadTransitAdmin();
...
function ensureContainerSearch(){
  ...
  if(!inp && sel.parentNode){
    ...
    inp.style.cssText = "width:160px;padding:7px 10px;border:1px solid #d1d5db;border-radius:8px;";
  ...
  function applyFilter(){
    ...
    for(var i = 0; i < opts.length; i++){
      ...
      var show = !q || val.toLowerCase().indexOf(q) >= 0 || txt.toLowerCase().indexOf(q) >= 0;
  ...
  inp.addEventListener("keydown", function(e){
    ...
    var exact = hits.filter(function(v){ return v.toLowerCase() === q.toLowerCase(); });
...
function boot(){
  ...
  console.log("transit_inbound.js ready v20260910a (hide already inbound)");
}
